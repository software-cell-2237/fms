...
@section('container')
			...
			<!-- BEGIN PAGE HEADER-->
			<h3 class="page-title">
			Level Four <small>Financial Management System</small>
			</h3>
			<div class="page-bar">
			...
			<div class="portlet box green ">
                <div class="portlet-title">
                    <div class="caption">
                        <i class="fa fa-gift"></i> Enter Level Four Data
                    </div>
                    <div class="tools">
...
<div class="row">
    <div class="col-md-12">
        ...
        <div class="portlet box blue-madison">
            <div class="portlet-title">
                <div class="caption">
                    <i class="fa fa-globe"></i>View Level Four Data
                </div>
                <div class="tools">
...
$(document).ready(function() {
    $('#main_code').on('change', function() {
        ...
        $.ajax({
            url: "{{ url('/levelFour/getControlCode') }}", // Update the URL with your controller route
            type: 'GET',
            data: {
    ...
    $('#control_code').on('change', function() {
        ...
        // Send the selected value to the controller using Ajax
        $.ajax({
            url: "{{ url('/levelFour/getSubCode')}}", // Update the URL with your controller route
            type: 'GET',
            data: {
            ...
            success: function(response) {
                var dropdown = $('#sub_code');
                dropdown.empty();  // Clear existing options

                $.each(response, function(index, item) {
                    //dropdown.append('<option value="''">' + Select 123 + '</option>');
                    dropdown.append('<option value="' + item.account_name + '">' + item.account_name + '</option>');
                });
            },
            error: function(xhr, status, error) {
    ...
    $('#sub_code').on('change', function() {
        ...
        // Send the selected value to the controller using Ajax
        $.ajax({
            url: "{{ url('/levelFour/list') }}", // Update the URL with your controller route
            type: 'GET',
            data: {
                sub_code: sub_code,
